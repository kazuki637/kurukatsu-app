...
export default function CircleMemberScheduleScreen({ route, navigation }) {
  ...
  const [circleName, setCircleName] = useState('スケジュール');
  ...
  useEffect(() => {
    const fetchCircle = async () => {
      if (circleId) {
        ...
        if (docSnap.exists()) {
          setCircleName(docSnap.data().name || 'スケジュール');
        }
      }
    };
    fetchCircle();
